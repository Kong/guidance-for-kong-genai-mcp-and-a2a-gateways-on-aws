...
The most fundamental comp



## The first Agent

To get started consider this [basic Agent](../assets/code/server_1.0.py). The Agent defines an AgentCard with has only one Skill. Here are some concise definitions provided by the **A2A Protocol** specification:
...
In another terminal send a request to it to get its Card:
```
...
As described in the Card, the Agent has only one SKill
