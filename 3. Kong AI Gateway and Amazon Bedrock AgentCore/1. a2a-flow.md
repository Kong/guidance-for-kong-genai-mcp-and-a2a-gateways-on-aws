...
* An [Agent Skill](https://a2a-protocol.org/v1.0.0/tutorials/python/3-agent-skills-and-card/#agent-skills) describes a specific capability or function the Agent can perform. It's a building block that tells clients what kinds of tasks the agent is good for. An [Agent Card](https://a2a-protocol.org/v1.0.0/tutorials/python/3-agent-skills-and-card/#agent-card) is a JSON document that an A2A Server makes available, typically at a [.well-known/agent-card.json endpoint](https://a2a-protocol.org/v1.0.0/topics/agent-discovery/#1-well-known-uri). It's like a digital business card for the agent.
...
``./agentcorekong/app/agentcorekong/pyptoject.toml``
...
'https://bedrock-agentcore.us-west-2.amazonaws.com/runtimes/arn%3Aaws%3Abedrock-agentcore%3Aus-west-2%3A481711488351%3Aruntime%2Fagentcorekong_agentcorekong-mU6f9tFT0B/invocations/?qualifier=DEFAULT' \


  'https://bedrock-agentcore.us-west-2.amazonaws.com/runtimes/arn%3Aaws%3Abedrock-agentcore%3Aus-west-2%3A481711488351%3Aruntime%2Fagentcorekong_agentcorekong-hJOstcE91q/invocations' 
...
agentcore deploy
```
aws logs tail /aws/bedrock-agentcore/runtimes/agentcorekong_agentcorekong-hJOstcE91q-DEFAULT \
  --follow \
  --region us-west-2
...
% aws bedrock-agentcore-control list-agent-runtimes --region us-west-2
{
    "agentRuntimes": [
        {
            "agentRuntimeArn": "arn:aws:bedrock-agentcore:us-east-2:481711488351:runtime/kongagentcore_kongagentcore-IcMtukH94z",
            "agentRuntimeId": "kongagentcore_kongagentcore-IcMtukH94z",
            "agentRuntimeVersion": "1",
            "agentRuntimeName": "kongagentcore_kongagentcore",
            "description": "AgentCore Runtime: kongagentcore_kongagentcore",
            "lastUpdatedAt": "2026-04-21T15:59:24.113215+00:00",
            "status": "READY"
        }
    ]
}
...
aws bedrock-agentcore-control get-agent-runtime \
  --agent-runtime-id kongagentcore_kongagentcore-xRHFSSFpX5 \
  --region us-west-2
...
aws iam simulate-principal-policy \
  --policy-source-arn arn:aws:iam::481711488351:user/cacquaviva-cli \
  --action-names bedrock:InvokeAgent \
  --resource-arns "arn:aws:bedrock-agentcore:us-east-2:481711488351:runtime/kongagentcore_kongagentcore-IcMtukH94z"


aws iam simulate-principal-policy \
  --policy-source-arn arn:aws:iam::481711488351:role/kaigateway-aws-podid-role \
  --action-names bedrock-agentcore:InvokeAgentRuntime \
  --resource-arns "arn:aws:bedrock-agentcore:us-west-2:481711488351:runtime/kongagentcore_kongagentcore-xRHFSSFpX5" \
  --region us-west-2
...
----- AgentCore Log
aws logs tail /aws/bedrock-agentcore/runtimes/kongagentcore_kongagentcore-xRHFSSFpX5-DEFAULT \
  --follow \
  --region us-west-2
-----
...
kubectl exec -ti $(kubectl get pod -n kong -o json | jq -r '.items[].metadata | select(.name | startswith("dataplane"))' | jq -r '.name') -n kong -- env
...
curl -X POST http://kong-dp.kong-demo.com/a2a \
  -H "Content-Type: application/json" \
  -H "apikey-a2a: abcde" \
...
kubectl run -n kong --rm=true -i --tty ubuntu --image=claudioacquaviva/ubuntu-awscli:0.6 -- /bin/bash


kubectl run -n kong \
  --overrides='{ "spec": { "serviceAccountName": "kaigateway-podid-sa" } }' \
  --rm=true \
  -i --tty ubuntu \
  --image=claudioacquaviva/ubuntu-awscli:0.5 -- /bin/bash




aws bedrock-agentcore list-agent-runtimes --region us-west-2 \
  --query 'agentRuntimes[?agentRuntimeName==`kongagentcore`].[agentRuntimeName,status]'
...
kubectl port-forward service/a2aserver 9000

curl -s http://kong-dp.kong-demo.com/.well-known/agent-card.json | jq


curl -X POST http://127.0.0.1:9000/agent1/a2a \
